...
def save_edit_record(summary_id, ai_original, final_text):
    """保存编辑记录并计算 diff"""
    ...
    conn.execute(
        '''INSERT INTO edit_records
           (id, summary_id, ai_original, final_text, edit_ratio, diff_paragraph, diff_sentence, diff_word)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?)''',
        (record_id, summary_id, ai_original, final_text, edit_ratio,
         json.dumps(diff_para), json.dumps(diff_sent), json.dumps(diff_word))
    )
    conn.commit()
    conn.close()
    return record_id
